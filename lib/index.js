...
function createMealFoodsTable(foods){
  const mealFoodsTable = $("#meal_food_table tbody")
    foods.forEach(function(food){
      const tableRow = document.createElement("tr")
      $(tableRow).addClass(`food-id-${food.id}`)
...
$(function(){
  ...
  $('#meal_selections').on('click', function (event) {
    ...
    var checkedFoods = $('#meal_food_table').find('input:checked')
    var foodName = checkedFoods.parent().siblings('td[id=fname]')[0].innerHTML
    var mealName = event.target.dataset.meal
    Meal.addFoodsToMeal(foodName, mealName)
    .then(function() {
      Meal.updateTable(mealName)
    })
  })
